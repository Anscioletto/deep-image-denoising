...
1. **Simple Autoencoder**: A shallow baseline with 2 convolutional layers.
2. **Deep Autoencoder (No Skip Connections)**: A deeper sequential architecture to test information bottlenecking and vanishing gradients.
3. **Deep Autoencoder (With Skip Connections)**: A U-Net style residual model designed to preserve high-frequency spatial features.
4. **Transfer Learning Model**: A pre-trained backbone loaded via `timm` and fine-tuned for dense pixel-level reconstruction.

---

##  Datasets & Noise Setup

* **Datasets**: Fashion-MNIST (1 channel) and CIFAR-10 / CIFAR-100 (3 channels, $32 \times 32$).
* **Corruptions**:
...
Run all cells sequentially to train the models, generate evaluation metrics, and render Grad-CAM visualizations.
